...
from air_compressor import compute_air_compressor
from helium_loop import compute_helium_loop
from helium_loop import compute_helium_hx
...
def main():
    ...
    # === Compute Air Compressor Conditions ===
    air_compressor_conditions = compute_air_compressor(precooler_conditions, params)
    
    print("\n--- Air Compressor Conditions ---")
    for key, value in air_compressor_conditions.items():
        print(f"{key}: {value:.3f}")
    
    # === Compute Helium Loop Conditions ===
    helium_conditions = compute_helium_loop(air_compressor_conditions, params)
    
    print("\n--- Helium Loop Conditions ---")
    for key, value in helium_conditions.items():
        print(f"{key}: {value:.3f}")
    
    # === Continue with other cycle components ===
    # hydrogen_conditions = compute_hydrogen_path(helium_hx_conditions, params)
